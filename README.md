...
**也別往上試更大的**。Qwen3.5-4B Q4_K_M 也測了，品質確實是另一個檔次（「我講話速度很快嗎」
它會正確回答聽不到你的語速，2B 是答非所問），但這台塞不下：

| 模型 / `-ngl` | 顯存 | 延遲 |
|---|---|---|
| 2B / 16（現用） | 1096 MiB | 0.58s |
| 4B / 99 | 2866 MiB | 0.50s |
| 4B / 20 | 1992 MiB | 0.99s |
| 4B / 14（最省） | 1586 MiB | 1.21s |

4B 用最省的 `-ngl 14` 起來之後只剩 4353 MiB，CosyVoice 要 4400，實跑兩輪兩輪都在合成
那步 OOM。要真的塞下得把 `-ngl` 降到 4 左右，延遲會超過 3 秒，比 llmshare 還慢。
**2B 是這台 8 GB 顯卡上地端 LLM 的天花板**，想要更好的答案就用 `--backend groq`。
...
- **合成的 OOM 會偽裝成別的錯誤**：`Calculated padded input size per channel: (3).
  Kernel size: (4)`。真正發生的是 CosyVoice 內部那條 LLM 執行緒先 OOM 死了，token 幾乎沒
  產出，卷積層才拿到 3 個 frame 報這個錯。看到這個訊息先查顯存，不要去追卷積。
- **`PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` 是必須的**，程式現在會自己設
  （一定要在 `import torch` 之前）。沒有它，顯存明明還剩一千多，卻會在配置 110 MiB 時
  OOM——那是碎片化，不是真的沒空間。實測差別很大：沒設的時候長回答常爆，設了之後
  連續四輪、語音長到 22 秒都沒事，餘裕還是一樣的一千三。
- **切句分次合成救不了 OOM**（量過：整段尖峰 4388 MiB、切句 4408 MiB，是雜訊）。
  合成的尖峰大約固定在常駐之上 1110 到 1140 MiB，跟文字長度沒什麼關係。
- **顯存不會隨對話輪數累積**（實測連跑八輪，剩餘顯存穩定在 1237 到 1247 MiB）。會失敗是因為
  餘裕本來就只有一千出頭，而合成尖峰要 900 到 1100，跟回答長度有關。所以 `:len` 調小
  不只是變快，也真的會降低 OOM 機率。
