...
def ask_llm(question, model, max_chars):
    prompt = (f"用正體中文口語回答，{max_chars} 個字以內，只回答問題本身，"
    ...
    return " ".join(r.stdout.split())[:max_chars * 2]
...
def main():
    ...
    ap.add_argument("--model", default="deepseek-v4-flash:0731",
                    help="llmshare 模型（llmshare models 看清單）")
